...
Finding $x = H^{-1}g$ is equivalent to solving this following block-linear
system for $x, y, z$.  The matrix $\mathcal{K}$ has dimension roughly $(Lp +
  2La) \times (Lp + 2La)$.  $D_D D_{xx}$, $D_D D_{zx} P$, $P^\top D_M D_{xz}$,
$P^\top D_M D_{zz} P$, and $D_x$ are block-diagonal or block-off-diagonal, and
$M$ is block-bidiagonal.

If we reorder the variables layer-by-layer as $(x_1, y_1, z_1, \dots, x_L, y_L,
  z_L)$, the matrix $\mathcal{K}$ becomes a block-banded matrix with bandwidth
proportional to $(p+a)$.  Solving a linear system with a banded matrix of size
$N$ and bandwidth $w$ requires $O(N w^2)$ operations. Here, $N \propto L$ and $w
  \propto (p+a)$. Therefore, we can compute $H^{-1}g$ in $O(L(p+a)^3)$ time. This
confirms that for tall-skinny networks, inverting the Hessian is linear in the
