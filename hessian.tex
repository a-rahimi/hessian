...
\maketitle \begin{abstract} We describe an exact algorithm for solving linear
  ...
  Hessian-inverse-vector products without storing the Hesssian or its inverse.  in
...
descent \cite{Watrous1987, Barnard1992}.  But large modern deep nets, using the
...
It has long been known that the product of the Hessian with a fixed vector (the
so-called Hessian-vector product) can be computed in time and storage that
scales linearly with the number of parameters in the network.  The method, which
is due to Pearlmutter \cite{pearlmutter94}, scales dramatically better than the
cubic scaling of the naive algorithm that first computes the Hessian matrix and
then multiplies it by the vector.  It idea is to produce a new network based on
the the original network whose gradient is the desired Hessian-vector product.
Given a way to compute the Hessian-vector product, one can then compute the
Hessian-inverse-vector product by, say Krylov iterations like Conjugate
Gradient. However, the quality of the result would then depend on the
...
the inverse of a block-bi-diagonal operator that represents the backpropoagation
...
networks are extruded layerwise: instead of have a few layers with many
...
Appendix \ref{sec:pearlmutter} shows that these operations are exactly the are
exactly  the operations performed in Pearlmutter's trick to compute the
...
transpose operation on block matrices. When appliekd to $\K$, it turns it into a
block-tri-diagonal matrix.  To get a better feel for this operation, denote the
$ij$th block of $\K$ by $\K_{ij}$ and the $uv$th sub-block of this block by
$\K_{ij,uv}$.  Similarly, $x$ denote a vector conformant with $\K$, with $x_j$
denoting the column vector that multiplies each block $\K_{\cdot j}$ and denote
the $v$th sub-block of $x_j$ by $x_{jv}$, which multiplies $\K_{\cdot j,\cdot
    v}$.  We say that $x$ is $j$-major because as we traverse the entries of $x$
from top to bottom, the index $j$ increments more slowly than the $v$ index.
...
then solve for $L y = b'$  using back subustitution, and finally solve for
