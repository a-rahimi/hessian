...
largely ignoring this complication and assume that difapproximations to these
operations are used.
...
\begin{align}
  ...
  \end{bmatrix} ^\top
  \begin{bmatrix}
    M^{-\top}P^\top D_M D_{zz} PM^{-1} & M^{-\top} P^\top D_M D_{xz} \\
    D_D D_{zx} P M^{-1}        & D_D D_{xx}
  \end{bmatrix}
  \begin{bmatrix}
    D_x \\ I
  \end{bmatrix}
  \\
  & =
  \underbrace{I+D_x^\top  D_x}_{\equiv A} +
  \underbrace{
    \begin{bmatrix} D_x \\ I
    \end{bmatrix}^\top
  }_{U^\top}
  \underbrace{
    \begin{bmatrix}
      M^{-\top}P^\top D_M D_{zz} PM^{-1} - I & M^{-\top} P^\top D_M D_{xz} \\
      D_D D_{zx} P M^{-1}        & D_D D_{xx} - I
    \end{bmatrix}
  }_{\equiv Q}
  \underbrace{
    \begin{bmatrix} D_x \\ I
...
\begin{align}
  H^{-1} &= \left(A+U^\top Q U\right)^{-1} = A^{-1} - A^{-1} U^\top \left(\underbrace{Q^{-1} + U A^{-1} U^\top}_{\equiv S_H}\right)^{-1} U A^{-1}.
\end{align}
This matrix can be efficiently applied to a vector without first forming it
because each of $U$, $A^{-1}$, and $S_H^{-1}$ can be applied efficiently to a vector.
The matrices $U$ and $A^{-1}$ can be formed and applied efficiently.
To apply the inverse of $S_H \equiv Q^{-1} + UA^{-1}U^\top$. we'll first 
compute and store the inverse of $Q$.  Denoting the blocks
...
\begin{align}
  ...
  Q_{11} &= M^{-\top}P^\top D_M D_{zz} PM^{-1} - I \\
  \label{eq:Q-blocks-12}
  Q_{12} &= M^{-\top}P^\top D_M D_{xz} \\
  \label{eq:Q-blocks-21}
  Q_{21} &= D_D D_{zx} P M^{-1} \\
...
the partitioned matrix inverse formula again invert $S_H$:
\[
  S_H^{-1} \equiv
  \begin{bmatrix}
...
\begin{enumerate}
  ...
  \item \textbf{Compute $\left[
    ...
    \begin{enumerate}
      ...
      \item Compute $g_{12}'' = \tilde{S}_{H_{12}} g_2'$.  This requires applying $G^{-\top}$ followed by $G^{-1}$ to $S_{H_{21}} S_{H_{11}}^{-1} g_2'$,
