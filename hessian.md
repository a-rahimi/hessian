...
the rate of progress. Preconditioning is the process of aligning $\nabla F$ with
the minimizer.  Preconditioning introduces a change of variables $x = G(y)$, and
performs gradient descent on $F(G(y))$ instead. The solution $y^*$ is then
...
= D_b \left( D_{xx} + D_{zx} N^{-1} D^{\tiny\searrow} \right) 
+ D_xM^{-1} B_b\left( D_{xz} + D_{zz} N^{-1} D^{\tiny\searrow} \right)
\end{align*}.
$$
