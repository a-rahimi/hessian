...
def train_epoch_hessian(model, train_loader, lr, device, epoch, 
                        use_hessian_every_n=1):
    """
...
def main():
    parser = argparse.ArgumentParser(description='Train MLP on ImageNet with Hessian preconditioning')
    ...
    parser.add_argument('--use-hessian', action='store_true',
                       help='use Hessian-preconditioned gradients')
    parser.add_argument('--hessian-freq', type=int, default=1,
                       help='apply Hessian every N batches')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.use_hessian:
            train_loss, train_acc, epoch_time, hess_time = train_epoch_hessian(
                model, train_loader, args.lr, device, epoch,
